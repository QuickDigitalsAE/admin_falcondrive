...
@section('title', 'Edit Car With Driver')
@section('page_title', 'Edit Car With Driver')
...
@section('breadcrumbs')
    <nav class="flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
        ...
        <i class="fas fa-chevron-right text-[10px] text-slate-400"></i>
        <span class="font-medium text-slate-700">Edit Record</span>
    </nav>
@endsection
    ...
    <section class="w-full pb-8">
        <div class="mx-auto w-full max-w-7xl">
            <div class="overflow-hidden rounded-[28px] border border-[#eadfbe] bg-white shadow-sm">
                <div class="border-b border-[#f0e6ca] bg-gradient-to-r from-[#fffaf0] to-[#fffdf8] px-4 py-5 sm:px-6">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="min-w-0">
                            ...
                            <h1 class="text-[28px] font-bold leading-tight text-slate-900">Edit Record</h1>
                            <p class="mt-1 text-sm text-slate-500">Update display content, SEO fields, linked cars, and the card image.</p>
                        </div>
                        <div class="flex flex-wrap gap-3">
                            <a href="{{ route('admin.car-with-drivers.show', $record->id) }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-5 py-3 text-sm font-semibold text-[#7d6220] shadow-sm transition hover:bg-[#fff8e8]">
                                <i class="fa-solid fa-eye mr-2 text-[13px]"></i>View Record
                            </a>
                ...
                <form action="{{ route('admin.car-with-drivers.update', $record->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="space-y-6 p-4 sm:p-6">
                    @method('PUT')
                    @include('admin.car-with-drivers._form', ['submitLabel' => 'Update Record'])
                </form>
            </div>
        </div>
    </section>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const defaultCardImage = @json($record->card_image_url ?: 'https://placehold.co/200x200/f8e8b2/5e450a?text=Card');
        ...
        cardImageInput?.addEventListener('change', function (event) {
            ...
            if (!file) {
                ...
                cardImageFileName.textContent = @json(!empty($record->card_image) ? basename($record->card_image) : 'No file selected');
                return;
            }
