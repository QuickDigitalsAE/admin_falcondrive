...
<div class="grid grid-cols-1 gap-5 xl:grid-cols-3">
    @foreach ([
        ...
        ['id' => 'name_ar', 'label' => 'Name AR', 'required' => true],
        ['id' => 'slug', 'label' => 'Slug', 'required' => true],
        ['id' => 'seo_title_en', 'label' => 'SEO Title EN', 'required' => true],
        ['id' => 'seo_title_ar', 'label' => 'SEO Title AR', 'required' => false],
    ] as $field)
        <div class="min-w-0">
            <div class="space-y-2">
                <div class="relative">
                    <input id="{{ $field['id'] }}" type="text" name="{{ $field['id'] }}" value="{{ old($field['id'], $lease->{$field['id']} ?? '') }}" placeholder="{{ $field['label'] }}"
    ...
    <div class="min-w-0 xl:col-span-3">
        <div class="space-y-2">
            ...
            <div class="resource-ckeditor-shell {{ $errors->has('description_en') ? 'is-invalid' : '' }}">
                <textarea id="description_en" name="description_en" rows="5">{{ old('description_en', $lease->description_en ?? '') }}</textarea>
            </div>
    ...
    <div class="min-w-0 xl:col-span-3">
        <div class="space-y-2">
            ...
            <div class="resource-ckeditor-shell {{ $errors->has('description_ar') ? 'is-invalid' : '' }}">
                <textarea id="description_ar" name="description_ar" rows="5">{{ old('description_ar', $lease->description_ar ?? '') }}</textarea>
            </div>
            @error('description_ar')<p class="px-1 text-xs font-medium text-red-600">{{ $message }}</p>@enderror
        </div>
    </div>
</div>
<div class="rounded-[22px] border border-dashed border-[#eadfbe] bg-[#fffdf8] px-4 py-4 sm:px-5"><div class="flex items-start gap-3"><div class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#f8e8b2] text-[#a27d20]"><i class="fa-solid fa-circle-info text-sm"></i></div><div class="min-w-0"><h3 class="text-sm font-semibold text-slate-800">Before saving</h3><p class="mt-1 text-sm text-slate-500">Keep lease naming and slug clean, and use SEO fields only where the record is intended for public display.</p></div></div></div>
<div class="mt-6 flex flex-col gap-3 border-t border-[#f0e6ca] pt-6 sm:flex-row sm:flex-wrap"><button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-[#d6ab3d] px-6 py-3 text-sm font-semibold text-white"><i class="fa-solid fa-floppy-disk mr-2 text-[13px]"></i>{{ $submitLabel }}</button>@if(!empty($lease?->id))<a href="{{ route('admin.lease.show', $lease->id) }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-6 py-3 text-sm font-semibold text-[#7d6220]"><i class="fa-solid fa-eye mr-2 text-[13px]"></i>View Lease</a>@endif<a href="{{ route('admin.lease') }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-6 py-3 text-sm font-semibold text-[#7d6220]"><i class="fa-solid fa-xmark mr-2 text-[13px]"></i>Cancel</a></div>
