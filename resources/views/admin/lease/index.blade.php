...
@section('breadcrumbs')
    <nav class="flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
        ...
        <span class="font-medium text-slate-700">Lease</span>
    </nav>
@endsection
        ...
        <div class="grid grid-cols-1 gap-3 xl:grid-cols-[1fr_auto]">
            ...
            <div class="rounded-2xl border border-[#eee4ca] bg-white/95 p-3 shadow-sm">
                <div class="flex flex-wrap items-center justify-start gap-2 xl:justify-end">
                    @can('Lease_Add')
                        <a href="{{ route('admin.lease.create') }}" class="inline-flex items-center rounded-xl bg-[#c79a2b] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-[#b4871d]"><i class="fa-solid fa-plus mr-2 text-[13px]"></i>Add Lease</a>
                    @endcan
                    @can('Lease_Delete')
                    ...
                    @endcan
                    @can('Lease_ViewAll')
                        <a id="exportCsvBtn" href="{{ route('admin.lease', ['is_export' => 1]) }}" class="inline-flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-[#eadfbe] bg-white text-[#87671c] shadow-sm transition hover:bg-[#fff8e7]" title="Export CSV"><i class="fa-solid fa-file-csv text-[14px]"></i></a>
                    @endcan
                </div>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-[#f2ead4]">
                    <thead class="sticky top-0 z-10 bg-[#fffaf0]">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Lease</th>
                    ...
                    <tbody id="leaseTableBody" class="divide-y divide-[#f6f0df] bg-white">
                        <tr><td colspan="4" class="px-6 py-12 text-center text-slate-500">Loading lease records...</td></tr>
                    </tbody>
                </table>
            </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const endpoint = @json(route('admin.lease'));
            ...
            function actionsHtml(item) {
                ...
                if (!item.deleted_at && permissions.can_delete) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100" data-url="${item.delete_url}" data-method="DELETE" data-confirm="Are you sure you want to delete this lease record?" data-action-type="delete" title="Delete"><i class="fa-solid fa-trash-can text-[13px]"></i></button>`);
                if (item.deleted_at && permissions.can_restore) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100" data-url="${item.restore_url}" data-method="PUT" data-confirm="Are you sure you want to restore this lease record?" data-action-type="restore" title="Restore"><i class="fa-solid fa-recycle text-[13px]"></i></button>`);
            ...
            function renderRows(items) {
                if (!items.length) {
                    leaseTableBody.innerHTML = `<tr><td colspan="4" class="px-6 py-12 text-center text-slate-500">No lease records found.</td></tr>`;
                    return;
                }
            ...
            function setLoading() { leaseTableBody.innerHTML = `<tr><td colspan="4" class="px-6 py-12 text-center text-slate-500"><div class="inline-flex items-center gap-2"><i class="fa-solid fa-spinner fa-spin text-[#b49543]"></i><span>Loading lease records...</span></div></td></tr>`; }
            ...
            async function fetchRecords() {
                ...
                try {
                    ...
                    if (!response.ok || !result.status) throw new Error(result.message || 'Failed to fetch lease records.');
                    ...
                    leaseTableBody.innerHTML = `<tr><td colspan="4" class="px-6 py-12 text-center text-red-500">${escapeHtml(error.message || 'Something went wrong.')}</td></tr>`;
                    paginationWrapper.innerHTML = '';
            ...
            async function submitAction(url, method, confirmText, actionType = 'default') {
                ...
                const result = await Swal.fire({
                    title: isDelete ? 'Delete Lease Record?' : isRestore ? 'Restore Lease Record?' : 'Are you sure?',
                    text: confirmText,
            ...
            leaseTableBody.addEventListener('click', function (event) { const btn = event.target.closest('.action-btn'); if (!btn) return; submitAction(btn.dataset.url, btn.dataset.method, btn.dataset.confirm, btn.dataset.actionType || 'default'); });
            fetchRecords();
        });
    </script>
@endpush
