...
@section('breadcrumbs')
    <nav class="flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
        ...
        <i class="fas fa-chevron-right text-[10px] text-slate-400"></i>
        <span class="font-medium text-slate-700">Edit Blog</span>
    </nav>
@endsection
    ...
    <section class="w-full pb-8">
        <div class="mx-auto w-full max-w-7xl">
            <div class="overflow-hidden rounded-[28px] border border-[#eadfbe] bg-white shadow-sm">
                <div class="border-b border-[#f0e6ca] bg-gradient-to-r from-[#fffaf0] to-[#fffdf8] px-4 py-5 sm:px-6">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="min-w-0">
                            ...
                            <h1 class="text-[28px] font-bold leading-tight text-slate-900">Edit Blog</h1>
                            <p class="mt-1 text-sm text-slate-500">Update blog titles, full descriptions, SEO metadata, schedule, and featured image.</p>
                        </div>

                        <div class="flex flex-wrap gap-3">
                            <a href="{{ route('admin.blogs.show', $blog->id) }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-5 py-3 text-sm font-semibold text-[#7d6220] shadow-sm transition hover:bg-[#fff8e8]">
                                <i class="fa-solid fa-eye mr-2 text-[13px]"></i>
                                View Blog
                            </a>
                            <a href="{{ route('admin.blogs') }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-5 py-3 text-sm font-semibold text-[#7d6220] shadow-sm transition hover:bg-[#fff8e8]">
                ...
                <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="space-y-6 p-4 sm:p-6">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
                        <div class="min-w-0"><div class="space-y-2"><div class="relative">
                            <input id="title_en" type="text" name="title_en" value="{{ old('title_en', $blog->title_en) }}" placeholder="Title English" class="peer w-full rounded-[18px] border {{ $errors->has('title_en') ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-[#e5d7b1] focus:border-[#caa23c] focus:ring-[#f7e9b5]' }} bg-[#fffdf8] px-4 pt-6 pb-2 text-sm text-slate-800 placeholder-transparent outline-none transition duration-200 focus:ring-4 min-h-[58px]">
                        ...
                        </div>@error('title_en')<p class="px-1 text-xs font-medium text-red-600">{{ $message }}</p>@enderror</div></div>

                        <div class="min-w-0"><div class="space-y-2"><div class="relative">
                            <input id="title_ar" type="text" name="title_ar" value="{{ old('title_ar', $blog->title_ar) }}" placeholder="Title Arabic" class="peer w-full rounded-[18px] border {{ $errors->has('title_ar') ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-[#e5d7b1] focus:border-[#caa23c] focus:ring-[#f7e9b5]' }} bg-[#fffdf8] px-4 pt-6 pb-2 text-sm text-slate-800 placeholder-transparent outline-none transition duration-200 focus:ring-4 min-h-[58px]">
                        ...
                        </div>@error('title_ar')<p class="px-1 text-xs font-medium text-red-600">{{ $message }}</p>@enderror</div></div>

                        <div class="min-w-0"><div class="space-y-2"><div class="relative">
                            <input id="slug" type="text" name="slug" value="{{ old('slug', $blog->slug) }}" placeholder="Slug" class="peer w-full rounded-[18px] border {{ $errors->has('slug') ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-[#e5d7b1] focus:border-[#caa23c] focus:ring-[#f7e9b5]' }} bg-[#fffdf8] px-4 pt-6 pb-2 text-sm text-slate-800 placeholder-transparent outline-none transition duration-200 focus:ring-4 min-h-[58px]">
                        ...
                        </div><p class="px-1 text-xs text-slate-500">Auto-generates from Title EN. You can still edit it manually. Allowed: `a-z`, `0-9`, `-`.</p>@error('slug')<p class="px-1 text-xs font-medium text-red-600">{{ $message }}</p>@enderror</div></div>

                        <div class="min-w-0"><div class="space-y-2"><div class="relative">
                            <input id="blog_schedule" type="datetime-local" name="blog_schedule" value="{{ old('blog_schedule', optional($blog->blog_schedule)->format('Y-m-d\TH:i')) }}" class="peer w-full rounded-[18px] border {{ $errors->has('blog_schedule') ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-[#e5d7b1] focus:border-[#caa23c] focus:ring-[#f7e9b5]' }} bg-[#fffdf8] px-4 pt-6 pb-2 text-sm text-slate-800 outline-none transition duration-200 focus:ring-4 min-h-[58px]">
                        ...
                        </div>@error('seo_title_en')<p class="px-1 text-xs font-medium text-red-600">{{ $message }}</p>@enderror</div></div>

                        <div class="min-w-0"><div class="space-y-2"><div class="relative">
                            <input id="seo_title_ar" type="text" name="seo_title_ar" value="{{ old('seo_title_ar', $blog->seo_title_ar) }}" placeholder="SEO Title Arabic" class="peer w-full rounded-[18px] border {{ $errors->has('seo_title_ar') ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-[#e5d7b1] focus:border-[#caa23c] focus:ring-[#f7e9b5]' }} bg-[#fffdf8] px-4 pt-6 pb-2 text-sm text-slate-800 placeholder-transparent outline-none transition duration-200 focus:ring-4 min-h-[58px]">
                        ...
                        </div>@error('seo_title_ar')<p class="px-1 text-xs font-medium text-red-600">{{ $message }}</p>@enderror</div></div>

                        <div class="min-w-0"><div class="space-y-2"><div class="relative">
                            <textarea id="seo_brief_en" name="seo_brief_en" rows="4" placeholder="SEO Brief English" class="peer w-full rounded-[18px] border {{ $errors->has('seo_brief_en') ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-[#e5d7b1] focus:border-[#caa23c] focus:ring-[#f7e9b5]' }} bg-[#fffdf8] px-4 pt-6 pb-3 text-sm text-slate-800 placeholder-transparent outline-none transition duration-200 focus:ring-4">{{ old('seo_brief_en', $blog->seo_brief_en) }}</textarea>
                        ...
                        </div>@error('seo_brief_en')<p class="px-1 text-xs font-medium text-red-600">{{ $message }}</p>@enderror</div></div>
                        ...
                        <div class="flex flex-col gap-5 lg:flex-row lg:items-center">
                            <div class="flex shrink-0 items-center gap-4">
                                <div class="flex h-24 w-24 items-center justify-center overflow-hidden rounded-2xl border border-dashed border-[#d8c79d] bg-white shadow-sm">
                                    <img id="blogPreview" src="{{ $blog->image_url ?: 'https://placehold.co/200x200/f8e8b2/5e450a?text=Blog' }}" alt="Blog Preview" class="h-full w-full object-cover">
                                </div>
                                <div>
                                    <h3 class="text-sm font-semibold text-slate-800">Blog Image</h3>
                                    <p class="mt-1 text-sm text-slate-500">Upload a new featured image for the blog.</p>
                                </div>
                            </div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="mt-3 flex flex-wrap items-center gap-3">
                                    <span id="fileName" class="rounded-full bg-white px-3 py-1 text-xs font-medium text-slate-500 shadow-sm ring-1 ring-[#eadfbe]">{{ $blog->image ? basename($blog->image) : 'No file selected' }}</span>
                                </div>
                    ...
                    <div class="flex flex-col gap-3 border-t border-[#f0e6ca] pt-6 sm:flex-row sm:flex-wrap">
                        ...
                        <a href="{{ route('admin.blogs.show', $blog->id) }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-6 py-3 text-sm font-semibold text-[#7d6220] shadow-sm transition hover:bg-[#fff8e8]">
                            <i class="fa-solid fa-eye mr-2 text-[13px]"></i>
                            View Blog
                        </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const defaultPreview = @json($blog->image_url ?: 'https://placehold.co/200x200/f8e8b2/5e450a?text=Blog');
        ...
        imageInput?.addEventListener('change', function (event) {
            ...
            if (!file) {
                ...
                fileName.textContent = '{{ $blog->image ? basename($blog->image) : 'No file selected' }}';
                return;
            }
