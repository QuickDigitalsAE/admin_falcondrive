...
@section('breadcrumbs')
    <nav class="flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
        ...
        <span class="font-medium text-slate-700">Cars</span>
    </nav>
@endsection
    ...
    <div class="grid grid-cols-1 gap-3 xl:grid-cols-[1fr_auto]">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                <div class="min-w-0 flex-1">
                    <div class="relative">
                        ...
                        <input type="text" id="searchInput" placeholder="Search by car name, brand, model or slug" class="w-full rounded-xl border border-[#eadfbe] bg-[#fffdf8] py-2.5 pl-11 pr-4 text-sm outline-none transition focus:border-[#d8bf79] focus:bg-white">
                    </div>
                </div>
        ...
        <div class="rounded-2xl border border-[#eee4ca] bg-white/95 p-3 shadow-sm">
            <div class="flex flex-wrap items-center justify-start gap-2 xl:justify-end">
                @can('Car_Add')
                    <a href="{{ route('admin.cars.create') }}" class="inline-flex items-center rounded-xl bg-[#c79a2b] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-[#b4871d]"><i class="fa-solid fa-plus mr-2 text-[13px]"></i>Add Car</a>
                @endcan
                @can('Car_Delete')
                ...
                @endcan
                <a id="exportCsvBtn" href="{{ route('admin.cars', ['is_export' => 1]) }}" class="inline-flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-[#eadfbe] bg-white text-[#87671c] shadow-sm transition hover:bg-[#fff8e7]" title="Export CSV"><i class="fa-solid fa-file-csv text-[14px]"></i></a>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-[#f2ead4]">
                <thead class="sticky top-0 z-10 bg-[#fffaf0]">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Car</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Brand</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Categories</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Pricing</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Stock</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Created</th>
                    </tr>
                </thead>
                <tbody id="carsTableBody" class="divide-y divide-[#f6f0df] bg-white">
                    <tr><td colspan="7" class="px-6 py-12 text-center text-slate-500">Loading cars...</td></tr>
                </tbody>
            </table>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const endpoint = @json(route('admin.cars'));
    ...
    function carHtml(car) {
        const image = car.main_image_url ? car.main_image_url : 'https://placehold.co/100x100/f8e8b2/5e450a?text=Car';
        return `<div class="flex items-center gap-3"><img src="${image}" alt="${escapeHtml(car.name_en)}" class="h-12 w-12 rounded-xl object-cover ring-2 ring-[#f4e3ab]"><div class="min-w-0"><p class="truncate text-sm font-semibold text-slate-800">${escapeHtml(car.name_en)}</p><p class="truncate text-sm text-slate-500">${escapeHtml(car.model || '')}</p><p class="truncate text-xs text-slate-400">${escapeHtml(car.slug || '')}</p></div></div>`;
    }
    function categoriesHtml(car) {
        const names = car.category_names || [];
        if (!names.length) return '<span class="text-sm text-slate-400">No categories</span>';
    ...
    function pricingHtml(car) {
        return `<div class="space-y-1 text-sm"><p class="text-slate-700"><span class="font-semibold">D:</span> ${escapeHtml(car.price_daily || '')}</p><p class="text-slate-500"><span class="font-semibold">W:</span> ${escapeHtml(car.price_weekly || '')}</p><p class="text-slate-500"><span class="font-semibold">M:</span> ${escapeHtml(car.price_monthly || '')}</p></div>`;
    }
    function stockHtml(car) {
        if (car.deleted_at) return `<span class="inline-flex whitespace-nowrap rounded-full bg-red-100 px-2.5 py-1 text-xs font-medium text-red-700">Deleted</span>`;
        return `<div class="space-y-1"><span class="inline-flex whitespace-nowrap rounded-full ${car.stock > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'} px-2.5 py-1 text-xs font-medium">Stock: ${escapeHtml(car.stock)}</span>${car.featured ? '<div><span class="inline-flex whitespace-nowrap rounded-full bg-amber-100 px-2.5 py-1 text-xs font-medium text-amber-700">Featured</span></div>' : ''}</div>`;
    }
    function actionsHtml(car) {
        const permissions = car.permissions || {};
        const buttons = [];
        if (permissions.can_view) buttons.push(`<a href="${car.show_url}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#eadfbe] bg-white text-[#87671c] transition hover:bg-[#fff8e7]" title="View"><i class="fa-solid fa-eye text-[13px]"></i></a>`);
        if (!car.deleted_at && permissions.can_edit) buttons.push(`<a href="${car.edit_url}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#d9c68f] bg-[#fff5d8] text-[#9b7a28] transition hover:bg-[#ffefc1]" title="Edit"><i class="fa-solid fa-pen text-[13px]"></i></a>`);
        if (!car.deleted_at && permissions.can_delete) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100" data-url="${car.delete_url}" data-method="DELETE" data-confirm="Are you sure you want to delete this car?" data-action-type="delete" title="Delete"><i class="fa-solid fa-trash-can text-[13px]"></i></button>`);
        if (car.deleted_at && permissions.can_restore) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100" data-url="${car.restore_url}" data-method="PUT" data-confirm="Are you sure you want to restore this car?" data-action-type="restore" title="Restore"><i class="fa-solid fa-recycle text-[13px]"></i></button>`);
    ...
    function setLoading() { carsTableBody.innerHTML = `<tr><td colspan="7" class="px-6 py-12 text-center text-slate-500"><div class="inline-flex items-center gap-2"><i class="fa-solid fa-spinner fa-spin text-[#b49543]"></i><span>Loading cars...</span></div></td></tr>`; }
    ...
    async function fetchCars() {
        ...
        try {
            ...
            if (!response.ok || !result.status) throw new Error(result.message || 'Failed to fetch cars.');
            renderRows(result.data.items || []); renderPagination(result.data.pagination || {}); renderMeta(result.data.pagination || {});
        } catch (error) {
            carsTableBody.innerHTML = `<tr><td colspan="7" class="px-6 py-12 text-center text-red-500">${escapeHtml(error.message || 'Something went wrong.')}</td></tr>`; paginationWrapper.innerHTML = ''; renderMeta({ from: 0, to: 0, total: 0 });
    ...
    async function submitAction(url, method, confirmText, actionType = 'default') {
        ...
        const result = await Swal.fire({ title: isDelete ? 'Delete Car?' : isRestore ? 'Restore Car?' : 'Are you sure?', text: confirmText || 'Please confirm this action.', icon: isDelete ? 'warning' : 'question', showCancelButton: true, confirmButtonText: isDelete ? 'Yes, Delete' : isRestore ? 'Yes, Restore' : 'Yes, Continue', cancelButtonText: 'Cancel', reverseButtons: true, focusCancel: true, background: '#ffffff', color: '#1e293b', confirmButtonColor: isDelete ? '#dc2626' : '#16a34a', cancelButtonColor: '#94a3b8', customClass: { popup: 'rounded-3xl', confirmButton: 'rounded-xl px-5 py-2', cancelButton: 'rounded-xl px-5 py-2' } });
        if (!result.isConfirmed) return; actionForm.action = url; actionFormMethod.value = method; actionForm.submit();
    }
    searchBtn.addEventListener('click', function(){ state.search = searchInput.value.trim(); state.page = 1; fetchCars(); });
    searchInput.addEventListener('keydown', function(e){ if (e.key === 'Enter') { e.preventDefault(); state.search = searchInput.value.trim(); state.page = 1; fetchCars(); } });
    resetBtn.addEventListener('click', function(){ state.search=''; state.is_deleted=0; state.page=1; searchInput.value=''; fetchCars(); });
    trashToggleBtn?.addEventListener('click', function(){ state.is_deleted = Number(state.is_deleted) === 1 ? 0 : 1; state.page = 1; fetchCars(); });
    paginationWrapper.addEventListener('click', function(e){ const btn = e.target.closest('.page-btn'); if (!btn || btn.disabled) return; const page = parseInt(btn.dataset.page || '1', 10); if (!page || page < 1 || page === state.page) return; state.page = page; fetchCars(); });
    carsTableBody.addEventListener('click', function(e){ const btn = e.target.closest('.action-btn'); if (!btn) return; submitAction(btn.dataset.url, btn.dataset.method, btn.dataset.confirm, btn.dataset.actionType || 'default'); });
    fetchCars();
});
</script>
@endpush
