...
@section('breadcrumbs')
    <nav class="flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
        ...
        <span class="font-medium text-slate-700">Roles</span>
    </nav>
@endsection
        ...
        <div class="grid grid-cols-1 gap-3 xl:grid-cols-[1fr_auto]">
                ...
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <div class="min-w-0 flex-1">
                        <div class="relative">
                            ...
                            <input type="text" id="searchInput" placeholder="Search by role name or role level"
            ...
            <div class="rounded-2xl border border-[#eee4ca] bg-white/95 p-3 shadow-sm">
                <div class="flex flex-wrap items-center justify-start gap-2 xl:justify-end">
                    ...
                    @endcan
                    @can('Role_ViewAll')
                        <a id="exportCsvBtn" href="{{ route('admin.roles', ['is_export' => 1]) }}"
                            class="inline-flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-[#eadfbe] bg-white text-[#87671c] shadow-sm transition hover:bg-[#fff8e7]"
                            title="Export CSV">
                ...
                <table class="min-w-full divide-y divide-[#f2ead4]">
                    <thead class="sticky top-0 z-10 bg-[#fffaf0]">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Level</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Permissions</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Users</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Created</th>
                        </tr>
                    </thead>
                    <tbody id="rolesTableBody" class="divide-y divide-[#f6f0df] bg-white">
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">Loading roles...</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const endpoint = @json(route('admin.roles'));
            ...
            function actionsHtml(role) {
                const buttons = [];
                if (role.permissions?.can_view) buttons.push(`<a href="${role.show_url}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#eadfbe] bg-white text-[#87671c] transition hover:bg-[#fff8e7]" title="View"><i class="fa-solid fa-eye text-[13px]"></i></a>`);
                if (!role.deleted_at && role.permissions?.can_edit) buttons.push(`<a href="${role.edit_url}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#d9c68f] bg-[#fff5d8] text-[#9b7a28] transition hover:bg-[#ffefc1]" title="Edit"><i class="fa-solid fa-pen text-[13px]"></i></a>`);
                if (!role.deleted_at && role.permissions?.can_delete) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100" data-url="${role.delete_url}" data-method="DELETE" data-confirm="Are you sure you want to delete this role?" data-action-type="delete" title="Delete"><i class="fa-solid fa-trash-can text-[13px]"></i></button>`);
                if (role.deleted_at && role.permissions?.can_restore) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100" data-url="${role.restore_url}" data-method="PUT" data-confirm="Are you sure you want to restore this role?" data-action-type="restore" title="Restore"><i class="fa-solid fa-recycle text-[13px]"></i></button>`);
            ...
            function renderPagination(pagination) {
                ...
                for (let page = 1; page <= pagination.last_page; page++) {
                    if (page === 1 || page === pagination.last_page || (page >= pagination.current_page - 1 && page <= pagination.current_page + 1)) buttons.push(`<button type="button" class="page-btn rounded-lg border px-3 py-2 text-sm ${page === pagination.current_page ? 'border-[#c79a2b] bg-[#c79a2b] text-white' : 'border-[#eadfbe] bg-white text-[#87671c] hover:bg-[#fff8e7]'}" data-page="${page}">${page}</button>`);
                    else if (page === pagination.current_page - 2 || page === pagination.current_page + 2) buttons.push(`<span class="px-1 text-slate-400">...</span>`);
                }
            ...
            function setLoading() { rolesTableBody.innerHTML = `<tr><td colspan="6" class="px-6 py-12 text-center text-slate-500"><div class="inline-flex items-center gap-2"><i class="fa-solid fa-spinner fa-spin text-[#b49543]"></i><span>Loading roles...</span></div></td></tr>`; }
            ...
            async function fetchRoles() {
                ...
                try {
                    ...
                    if (!response.ok || !result.status) throw new Error(result.message || 'Failed to fetch roles.');
                    renderRows(result.data.items || []); renderPagination(result.data.pagination || {}); renderMeta(result.data.pagination || {});
                } catch (error) {
                    rolesTableBody.innerHTML = `<tr><td colspan="6" class="px-6 py-12 text-center text-red-500">${escapeHtml(error.message || 'Something went wrong.')}</td></tr>`; paginationWrapper.innerHTML = ''; renderMeta({from:0,to:0,total:0});
                } finally { state.loading = false; }
            }
            async function submitAction(url, method, confirmText, actionType = 'default') {
                const result = await Swal.fire({ title: actionType === 'delete' ? 'Delete Role?' : 'Restore Role?', text: confirmText, icon: actionType === 'delete' ? 'warning' : 'question', showCancelButton: true, confirmButtonText: actionType === 'delete' ? 'Yes, Delete' : 'Yes, Restore', cancelButtonText: 'Cancel', reverseButtons: true, focusCancel: true, confirmButtonColor: actionType === 'delete' ? '#dc2626' : '#16a34a', cancelButtonColor: '#94a3b8', customClass: { popup: 'rounded-3xl', confirmButton: 'rounded-xl px-5 py-2', cancelButton: 'rounded-xl px-5 py-2' } });
                if (!result.isConfirmed) return; actionForm.action = url; actionFormMethod.value = method; actionForm.submit();
            }
            const urlParams = new URLSearchParams(window.location.search);
            state.search = urlParams.get('search') || ''; state.is_deleted = urlParams.get('is_deleted') === '1' ? 1 : 0; state.page = parseInt(urlParams.get('page') || '1', 10); searchInput.value = state.search;
            searchBtn.addEventListener('click', function () { state.search = searchInput.value.trim(); state.page = 1; fetchRoles(); });
            searchInput.addEventListener('keydown', function (e) { if (e.key === 'Enter') { e.preventDefault(); state.search = searchInput.value.trim(); state.page = 1; fetchRoles(); } });
            resetBtn.addEventListener('click', function () { state.search = ''; state.is_deleted = 0; state.page = 1; searchInput.value = ''; fetchRoles(); });
            if (trashToggleBtn) trashToggleBtn.addEventListener('click', function () { state.is_deleted = Number(state.is_deleted) === 1 ? 0 : 1; state.page = 1; fetchRoles(); });
            paginationWrapper.addEventListener('click', function (e) { const btn = e.target.closest('.page-btn'); if (!btn || btn.disabled) return; const page = parseInt(btn.dataset.page || '1', 10); if (!page || page < 1 || page === state.page) return; state.page = page; fetchRoles(); });
            rolesTableBody.addEventListener('click', function (e) { const btn = e.target.closest('.action-btn'); if (!btn) return; submitAction(btn.dataset.url, btn.dataset.method, btn.dataset.confirm, btn.dataset.actionType || 'default'); });
            fetchRoles();
        });
    </script>
@endpush
