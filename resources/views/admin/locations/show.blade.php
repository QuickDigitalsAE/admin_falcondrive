            ...
            <div class="overflow-hidden rounded-[28px] border border-[#eadfbe] bg-white shadow-sm">
                <div class="border-b border-[#f0e6ca] bg-gradient-to-r from-[#fffaf0] to-[#fffdf8] px-4 py-5 sm:px-6">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div class="flex flex-wrap gap-3">@if(is_null($location->deleted_at))<a href="{{ route('admin.locations.edit', $location->id) }}" class="inline-flex items-center justify-center rounded-2xl bg-[#d6ab3d] px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#c59626]"><i class="fa-solid fa-pen-to-square mr-2 text-[13px]"></i>Edit Location</a>@endif<a href="{{ route('admin.locations') }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-5 py-3 text-sm font-semibold text-[#7d6220] shadow-sm transition hover:bg-[#fff8e8]"><i class="fa-solid fa-arrow-left mr-2 text-[13px]"></i>Back to List</a></div>
                    </div>
                </div>
                <div class="p-4 sm:p-6">
                    <div class="rounded-[24px] border border-[#eadfbe] bg-white p-5 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div class="rounded-2xl border border-[#f0e6ca] bg-[#fffdf8] p-4 sm:col-span-2"><p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#b89a4c]">Cars</p><div class="mt-2 flex flex-wrap gap-2">@forelse($location->cars as $car)<span class="inline-flex rounded-full border border-[#ead39a] bg-[#fff4d6] px-3 py-1 text-xs font-semibold text-[#7d6220]">{{ $car->name_en }}{{ $car->model ? ' (' . $car->model . ')' : '' }}</span>@empty<span class="text-sm font-semibold text-slate-400">No cars linked.</span>@endforelse</div></div>
                            ...
                            <div class="rounded-2xl border border-[#f0e6ca] bg-[#fffdf8] p-4"><p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#b89a4c]">SEO Brief AR</p><p class="mt-2 whitespace-pre-line break-words text-right text-sm font-semibold leading-8 text-slate-900 [overflow-wrap:anywhere]" dir="rtl">{{ $location->seo_brief_ar ?: 'N/A' }}</p></div>
                        </div>
                    </div>
                    <div class="mt-6 flex flex-col gap-3 border-t border-[#f0e6ca] pt-6 sm:flex-row sm:flex-wrap">@if(is_null($location->deleted_at))<a href="{{ route('admin.locations.edit', $location->id) }}" class="inline-flex items-center justify-center rounded-2xl bg-[#d6ab3d] px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#c59626]"><i class="fa-solid fa-pen-to-square mr-2 text-[13px]"></i>Edit Location</a>@endif<a href="{{ route('admin.locations') }}" class="inline-flex items-center justify-center rounded-2xl border border-[#eadfbe] bg-white px-6 py-3 text-sm font-semibold text-[#7d6220] shadow-sm transition hover:bg-[#fff8e8]"><i class="fa-solid fa-list mr-2 text-[13px]"></i>All Locations</a></div>
                </div>
            </div>
        </div>
    </section>
