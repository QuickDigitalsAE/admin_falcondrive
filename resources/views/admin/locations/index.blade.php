...
@section('breadcrumbs')
    <nav class="flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
        ...
        <span class="font-medium text-slate-700">Locations</span>
    </nav>
@endsection
        ...
        <div class="grid grid-cols-1 gap-3 xl:grid-cols-[1fr_auto]">
            ...
            <div class="rounded-2xl border border-[#eee4ca] bg-white/95 p-3 shadow-sm">
                <div class="flex flex-wrap items-center justify-start gap-2 xl:justify-end">
                    ...
                    @endcan
                    @can('Location_ViewAll')
                        <a id="exportCsvBtn" href="{{ route('admin.locations', ['is_export' => 1]) }}"
                            class="inline-flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-[#eadfbe] bg-white text-[#87671c] shadow-sm transition hover:bg-[#fff8e7]"
                            title="Export CSV">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const endpoint = @json(route('admin.locations'));
            ...
            function actionsHtml(item) {
                ...
                if (!item.deleted_at && permissions.can_delete) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100" data-url="${item.delete_url}" data-method="DELETE" data-confirm="Are you sure you want to delete this location?" data-action-type="delete" title="Delete"><i class="fa-solid fa-trash-can text-[13px]"></i></button>`);
                if (item.deleted_at && permissions.can_restore) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100" data-url="${item.restore_url}" data-method="PUT" data-confirm="Are you sure you want to restore this location?" data-action-type="restore" title="Restore"><i class="fa-solid fa-recycle text-[13px]"></i></button>`);
            ...
            async function submitAction(url, method, confirmText, actionType = 'default') {
                ...
                const result = await Swal.fire({
                    title: isDelete ? 'Delete Location?' : isRestore ? 'Restore Location?' : 'Are you sure?',
                    text: confirmText,
            ...
            locationsTableBody.addEventListener('click', function (event) { const btn = event.target.closest('.action-btn'); if (!btn) return; submitAction(btn.dataset.url, btn.dataset.method, btn.dataset.confirm, btn.dataset.actionType || 'default'); });
            fetchRecords();
        });
    </script>
@endpush
