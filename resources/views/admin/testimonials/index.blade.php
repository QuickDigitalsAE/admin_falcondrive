...
@section('breadcrumbs')
    <nav class="flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
        ...
        <span class="font-medium text-slate-700">Testimonials</span>
    </nav>
@endsection
        ...
        <div class="grid grid-cols-1 gap-3 xl:grid-cols-[1fr_auto]">
                ...
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <div class="min-w-0 flex-1">
                        <div class="relative">
                            ...
                            <input type="text" id="searchInput" placeholder="Search testimonials by name or description"
            ...
            <div class="rounded-2xl border border-[#eee4ca] bg-white/95 p-3 shadow-sm">
                <div class="flex flex-wrap items-center justify-start gap-2 xl:justify-end">
                    ...
                    @endcan
                    @can('Testimonial_ViewAll')
                        <a id="exportCsvBtn" href="{{ route('admin.testimonials', ['is_export' => 1]) }}"
                            class="inline-flex h-[42px] w-[42px] items-center justify-center rounded-xl border border-[#eadfbe] bg-white text-[#87671c] shadow-sm transition hover:bg-[#fff8e7]"
                            title="Export CSV">
                ...
                <table class="min-w-full divide-y divide-[#f2ead4]">
                    <thead class="sticky top-0 z-10 bg-[#fffaf0]">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Testimonial</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Preview</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase text-[#9d8750]">Created</th>
                        </tr>
                    </thead>
                    <tbody id="testimonialsTableBody" class="divide-y divide-[#f6f0df] bg-white">
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-500">Loading testimonials...</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const endpoint = @json(route('admin.testimonials'));
            ...
            function actionsHtml(item) {
                ...
                if (!item.deleted_at && permissions.can_delete) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100" data-url="${item.delete_url}" data-method="DELETE" data-confirm="Are you sure you want to delete this testimonial?" data-action-type="delete" title="Delete"><i class="fa-solid fa-trash-can text-[13px]"></i></button>`);
                if (item.deleted_at && permissions.can_restore) buttons.push(`<button type="button" class="action-btn inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100" data-url="${item.restore_url}" data-method="PUT" data-confirm="Are you sure you want to restore this testimonial?" data-action-type="restore" title="Restore"><i class="fa-solid fa-recycle text-[13px]"></i></button>`);
            ...
            function renderRows(items) {
                if (!items.length) {
                    testimonialsTableBody.innerHTML = `<tr><td colspan="4" class="px-6 py-12 text-center text-slate-500">No testimonials found.</td></tr>`;
                    return;
                }
            ...
            function setLoading() { testimonialsTableBody.innerHTML = `<tr><td colspan="4" class="px-6 py-12 text-center text-slate-500"><div class="inline-flex items-center gap-2"><i class="fa-solid fa-spinner fa-spin text-[#b49543]"></i><span>Loading testimonials...</span></div></td></tr>`; }
            ...
            async function fetchRecords() {
                ...
                try {
                    ...
                    if (!response.ok || !result.status) throw new Error(result.message || 'Failed to fetch testimonials.');
                    ...
                    testimonialsTableBody.innerHTML = `<tr><td colspan="4" class="px-6 py-12 text-center text-red-500">${escapeHtml(error.message || 'Something went wrong.')}</td></tr>`;
                    paginationWrapper.innerHTML = '';
            ...
            async function submitAction(url, method, confirmText, actionType = 'default') {
                ...
                const result = await Swal.fire({
                    title: isDelete ? 'Delete Testimonial?' : isRestore ? 'Restore Testimonial?' : 'Are you sure?',
                    text: confirmText,
            ...
            testimonialsTableBody.addEventListener('click', function (event) { const btn = event.target.closest('.action-btn'); if (!btn) return; submitAction(btn.dataset.url, btn.dataset.method, btn.dataset.confirm, btn.dataset.actionType || 'default'); });
            fetchRecords();
        });
    </script>
@endpush
