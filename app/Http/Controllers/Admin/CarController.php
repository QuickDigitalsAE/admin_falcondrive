...
class CarController extends Controller
{
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            $items = $cars->getCollection()->map(function (Car $car) use ($authUser) {
                return [
                    ...
                    'full_insurance_daily' => $car->full_insurance_daily,
                    'full_insurance_weekly' => $car->full_insurance_weekly,
                    'full_insurance_monthly' => $car->full_insurance_monthly,
                    'baby_seat_daily' => $car->baby_seat_daily,
                    'baby_seat_weekly' => $car->baby_seat_weekly,
                    'baby_seat_monthly' => $car->baby_seat_monthly,
                    'waiver_daily' => $car->waiver_daily,
                    'waiver_weekly' => $car->waiver_weekly,
                    'waiver_monthly' => $car->waiver_monthly,
    ...
    private function validationRules(Request $request, ?int $carId = null): array
    {
        return [
            ...
            'full_insurance_daily' => ['nullable', 'string', 'max:255'],
            'full_insurance_weekly' => ['nullable', 'string', 'max:255'],
            'full_insurance_monthly' => ['nullable', 'string', 'max:255'],
            'additional_driver_amount' => ['nullable', 'string', 'max:255'],
            'baby_seat_daily' => ['nullable', 'string', 'max:255'],
            'baby_seat_weekly' => ['nullable', 'string', 'max:255'],
            'baby_seat_monthly' => ['nullable', 'string', 'max:255'],
            'deposit_amount' => ['nullable', 'string', 'max:255'],
            'waiver_daily' => ['nullable', 'string', 'max:255'],
            'waiver_weekly' => ['nullable', 'string', 'max:255'],
            'waiver_monthly' => ['nullable', 'string', 'max:255'],
    ...
    private function buildPayload(array $validated, Request $request, ?Car $car = null): array
    {
        return [
            ...
            'full_insurance_daily' => $validated['full_insurance_daily'] ?? null,
            'full_insurance_weekly' => $validated['full_insurance_weekly'] ?? null,
            'full_insurance_monthly' => $validated['full_insurance_monthly'] ?? null,
            'additional_driver_amount' => $validated['additional_driver_amount'] ?? null,
            'baby_seat_daily' => $validated['baby_seat_daily'] ?? null,
            'baby_seat_weekly' => $validated['baby_seat_weekly'] ?? null,
            'baby_seat_monthly' => $validated['baby_seat_monthly'] ?? null,
            'deposit_amount' => $validated['deposit_amount'] ?? null,
            'waiver_daily' => $validated['waiver_daily'] ?? null,
            'waiver_weekly' => $validated['waiver_weekly'] ?? null,
            'waiver_monthly' => $validated['waiver_monthly'] ?? null,
    ...
    private function exportCars($query, bool $isDeleted)
    {
        ...
        return response()->stream(function () use ($cars, $isDeleted) {
            ...
            $headers = ['ID', 'Name EN', 'Name AR', 'Brand', 'Model', 'Slug', 'Price Daily', 'Price Weekly', 'Price Monthly', 'Full Insurance Daily', 'Full Insurance Weekly', 'Full Insurance Monthly', 'Baby Seat Daily', 'Baby Seat Weekly', 'Baby Seat Monthly', 'Waiver Daily', 'Waiver Weekly', 'Waiver Monthly', 'Vehicle Group ID', 'Tariff Group ID', 'Stock', 'Featured', 'Created At'];
            ...
            foreach ($cars as $car) {
                $row = [
                    ...
                    $car->price_weekly,
                    $car->price_monthly,
                    $car->full_insurance_daily,
                    $car->full_insurance_weekly,
                    $car->full_insurance_monthly,
                    $car->baby_seat_daily,
                    $car->baby_seat_weekly,
                    $car->baby_seat_monthly,
                    $car->waiver_daily,
                    $car->waiver_weekly,
                    $car->waiver_monthly,
